...
const NavBar=(props)=>{
  const {dispatch,blogCurrentNav,blogNavButtons}=props
  ...
  return (
    <Element id="blogNavBar" onScroll={handleScroll} className={classnames(styles.container,{[styles.top]:isTop&&direction==='down',[styles.top1]:isTop&&direction==='up'})}>
      {blogNavButtons.map((item,index)=>(
        <div onClick={()=>handleClick(item,index)} key={index} className={classnames(styles.menuItem,{[styles.activeItem]:blogCurrentNav===index})}>
          {item}
