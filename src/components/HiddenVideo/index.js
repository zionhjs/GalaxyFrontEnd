...
const Video=(props)=>{
  ...
  const close=useCallback(()=>{
    dispatch({type:'animationHidden/closeVideo'})
  },[])
  const next=useCallback(()=>{
    dispatch({type:'animationHidden/next'})
  },[])
  const pre=useCallback(()=>{
    dispatch({type:'animationHidden/pre'})
  },[])
  return visible ? (
...
export default connect(({animationHidden:{videoVisible,currentItem}})=>({visible:videoVisible,currentItem}))(Video)
